...
module.exports = {
    handel: async function (args) {
        ...
        if (!args._[1]) {
            throw 'Name of seeder is required';
        }
        
        var name = args._[1];
        var dir = path.resolve(process.cwd(), 'src', 'database', 'seeders');
        ...
        return new Promise(async function (resolve, reject) {
            ...
            fs.readFile(path.resolve(__dirname, '..', 'generator', 'seeder.tpl'), 'utf8', function (err, data) {
                if (err) reject(err);
